/*	dynArr.c: Dynamic Array implementation.
	code credit: OSU CS 261, Spring 2015 */
#include <assert.h>
#include <stdlib.h>
#include <stdio.h>
...
void addHeap(DynArr *heap, TYPE val)
{
    ...
    //if last action was sorting, heap ordering is not in place, adjust heap
    _buildHeap(heap);
    //a function that adds to array and sets capacity if necessary
    addDynArr(heap, val);
    //checks position and percolate up
    while(pos >= 0){
        parent = (pos - 1 ) / 2;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
    ...
    if(rightIndex < max){
        ...
        int small = _smallerIndexHeap(heap, leftIndex, rightIndex);

        //compare smallest child to pos if necessary swap and call _adjustHeap
        ...
        if(leftIndex < max){
        //since we know that this has one child we
        //can simply compare child to pos
...
void sortHeap(DynArr *heap)
{
   /* FIXME */
    ...
    for(int i = max - 1; i >= 0; i--) {
        ...
        // for loop decrements last position to get new last
    }
}
